...
[CustomEditor(typeof(SpriteAnimPreset))]
[CanEditMultipleObjects]
public class SpriteAnimPresetInspector : Editor {
    public override void OnInspectorGUI() {
        base.OnInspectorGUI();

        GUILayout.Space(10);
        if (GUILayout.Button("Set prefab and then click")) {
            InitializePresetData(target as SpriteAnimPreset);
        }
    }

    private void InitializePresetData(SpriteAnimPreset preset) {
        preset.datas.Clear();
